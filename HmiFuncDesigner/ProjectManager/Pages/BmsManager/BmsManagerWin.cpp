...
const int MaxTagIOTableColumns = BmsTableModel::MaxColumns;

Qt::ItemFlags BmsTableModel::flags(const QModelIndex &index) const
{
...
QVariant BmsTableModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid() || index.row() < 0 || index.row() >= m_tagRows.count() || index.column() < 0 || index.column() >= MaxTagIOTableColumns) {
        return QVariant();
    }
...
QVariant BmsTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
int BmsTableModel::rowCount(const QModelIndex &index) const
{
    return index.isValid() ? 0 : m_tagRows.count();
}

int BmsTableModel::columnCount(const QModelIndex &index) const
{
    return index.isValid() ? 0 : MaxTagIOTableColumns;
}

bool BmsTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (!index.isValid() || role != Qt::EditRole || index.row() < 0 || index.row() >= m_tagRows.count() || index.column() < 0 ||
        index.column() >= MaxTagIOTableColumns) {
        return false;
    }
...
bool BmsTableModel::insertRows(int row, int count, const QModelIndex &)
{
    if (count < 1) {
        return false;
    }
...
bool BmsTableModel::removeRows(int row, int count, const QModelIndex &)
{
    if (count < 1) {
        return false;
    }
...
void BmsTableModel::AppendRow(QStringList rowDat)
{
...
void BmsTableModel::AppendRows(QVector<QStringList> rowsDat)
{
    if (rowsDat.size() < 1) {
        return;
    }
...
void BmsTableModel::InsertRow(int i, QStringList rowDat)
{
...
QStringList BmsTableModel::GetRow(int i)
{
    QStringList it;
    if (i < m_tagRows.size()) {
...
void BmsTableModel::UpdateRow(int i, QStringList rowDat)
{
    if (i < m_tagRows.size()) {
...
void BmsTableModel::UpdateRows(QVector<QStringList> rowsDat)
{
    m_tagRows = rowsDat;
...
QTableWidgetEx::QTableWidgetEx(QWidget *parent) : QTableView(parent)
{
    ...
    m_pBmsTableModel = new BmsTableModel();
    // 初始化变量表控件
    initTagsTable();
...
{
    if (m_pBmsTableModel != NULL) {
        delete m_pBmsTableModel;
        m_pBmsTableModel = NULL;
    }
}
...
{
    this->setModel(m_pBmsTableModel);
    ...
    this->setColumnWidth(4, 100); // 数据源
    this->setColumnWidth(5, 60); // 单位
    this->setColumnWidth(6, 80); // 缩放计算参数
    this->setColumnWidth(7, 80); // 功能码
    this->setColumnWidth(8, 80); // Redis计算变量描述
    this->setColumnWidth(9, 80);  // 变量描述
    ...
    for (int col = 0; col < BmsTableModel::MaxColumns; col++) {
        QVariant headerData = m_pBmsTableModel->headerData(col, Qt::Horizontal);
...
{
    m_pBmsTableModel->removeRows(0, m_pBmsTableModel->rowCount());
}
...
void QTableWidgetEx::updateTable()
{
    ...
    foreach (QSharedPointer<Tag> pTagObj, QSoftCore::getCore()->getProjectCore()->m_tagMgr->m_vecTags) {
        ...
        BmsTag *devTagPtr = dynamic_cast<BmsTag *>(pTagObj.data());
    ...
    m_pBmsTableModel->AppendRows(rowsDat);
}
...
void QTableWidgetEx::setRowData(QStringList &rowDat, Tag *pObj)
{
    ...
    BmsTag *pDevTagObj = static_cast<BmsTag *>(pObj);
    ...
    if (pDevTagObj->m_devType == "SYSTEM") {
        ...
        rowDat << pDevTagObj->m_name;
    }
    rowDat << pDevTagObj->m_addrOffset;
    rowDat << pDevTagObj->m_dataType;
    rowDat << QString::number(pDevTagObj->m_dataSource);
    rowDat << pDevTagObj->m_unit;
    rowDat << pDevTagObj->m_rate;
    rowDat << pDevTagObj->m_functionCodes;
    rowDat << pDevTagObj->m_formula;
...
void QTableWidgetEx::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    foreach (QModelIndex index, listModelIndex) {
        QStringList rowDat = m_pBmsTableModel->m_tagRows.at(index.row());
...
void QTableWidgetEx::onEditTag()
{
    ...
    QStringList rowDat = m_pBmsTableModel->m_tagRows.at(iRow);
    int iTagID         = rowDat.at(0).toInt();
    if (rowDat.at(1).startsWith(QString("$"))) {
        return; // 系统变量不可以编辑
    }
    ...
    for (int i = 0; i < tagList.count(); i++) {
        ...
        if (pTagObj->m_id == iTagID) {
            BmsEditDialog dlg(this, pTagObj.data());
            dlg.updateUI();
...
void QTableWidgetEx::onExportToCsv()
{
    ...
    QString szSaveCsvPath =
        QFileDialog::getExistingDirectory(this, tr("选择导出csv路径"), szDirPath, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (szSaveCsvPath == "") {
        return;
    }
    ...
    QStringList header;
    header << tr("ID") << tr("名称") << tr("单位") << tr("地址类型") << tr("地址偏移") << tr("地址类型2") << tr("地址偏移2") << tr("数据类型")
           << tr("读写类型") << tr("描述") << tr("所属组") << tr("设备类型");

    QtCSV::Writer::write(
        filepath, varData, QString(","), QString("\""), QtCSV::Writer::REWRITE, header, QStringList(), QTextCodec::codecForName("GB18030"));
}
...
{
    QString szDirPath     = QCoreApplication::applicationDirPath();
    QString szSaveCsvFile = QFileDialog::getOpenFileName(this, tr("选择csv文件"), szDirPath, tr("csv file (*.csv)"));
    if (szSaveCsvFile == "") {
        return;
    }

    QList<QStringList> data = QtCSV::Reader::readToList(szSaveCsvFile, QString(","), QString("\""), QTextCodec::codecForName("GB18030"));
...
BmsManagerWin::BmsManagerWin(QWidget *parent) : QWidget(parent)
{
...
bool BmsManagerWin::event(QEvent *ev)
{
